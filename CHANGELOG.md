...
## 2026-09-10 — full_report.php แสดงยุทธศาสตร์/ตัวชี้วัดทุกโครงการ
- `full_report.php`: ปรับการแสดงผลยุทธศาสตร์และตัวชี้วัดให้แสดงทุกโครงการ (ไม่ซ่อนเมื่อไม่มีข้อมูล)
- แสดง "-" เมื่อยุทธศาสตร์หรือตัวชี้วัดว่างเปล่า
...
- ปรับ `full_report.php`: แต่ละโครงการแสดงเป็น card ประกอบด้วย
  ...
  - หน่วยงาน / ผู้รับผิดชอบหลัก / ผู้รับผิดชอบร่วม
  - งบประมาณที่ได้รับ / ใช้ไป / ร้อยละ
  - ยุทธศาสตร์ / ตัวชี้วัด KPI / OKR (ถ้ามี)
  - สรุปผลการดำเนินงาน / กิจกรรมที่ดำเนินการ / ปัญหาอุปสรรค
  - รูปภาพกิจกรรม / วิดีโอ / เอกสาร / รายงาน (แสดงเป็นลิงก์)
- ดึงข้อมูลยุทธศาสตร์จาก `project_strategic_issues` + ตัวชี้วัดจาก `project_kpis` + OKR จาก `okr_projects` ตามรหัส/ชื่อโครงการ
- รองรับตัวกรองปีงบประมาณ + ขอบเขตตามสิทธิ์, print-friendly CSS, ปุ่ม Print Preview + ส่งออก PDF
- ทดสอบบน server: `full_report.php?year=2569` โหลดสำเร็จ แสดงรายละเอียดครบทุกโครงการ
- Deploy ขึ้น server แล้ว: `full_report.php`
...
- Deploy ขึ้น server แล้ว: `project_form.php`
...
- Deploy ขึ้น server แล้ว: `menu.php`

## 2026-08-10 — ผู้ประสานงานหน่วยงาน (office) กำหนดผู้ใช้ได้ 2 ระดับ: office และ user
- `users.php`: office กำหนดบทบาทได้ทั้ง "ผู้ใช้ทั่วไป (user)" และ "ผู้ประสานงานหน่วยงาน (office)" ในหน่วยงานตนเอง (เดิมจำกัดเฉพาะ user)
- แก้ทุกจุดตรวจสอบสิทธิ์ office: AJAX โหลดข้อมูล, บันทึก (สร้าง/แก้ไข), เปลี่ยนสถานะ, รีเซ็ตรหัสผ่าน — อนุญาตเฉพาะบทบาท user/office ในหน่วยงานตนเอง
- modal ฟอร์ม: office เห็น dropdown บทบาท {user, office} แทนช่อง locked; ยังคงล็อกหน่วยงาน (สังกัด)
- ตัวกรองตามบทบาท: office เห็นแท็บ ทั้งหมด / ผู้ประสานงานหน่วยงาน / ผู้ใช้ทั่วไป (พร้อมจำนวน)
- admin/plan ยังคงจัดการทุกบทบาท; office ไม่สามารถสร้าง/แก้ไข admin หรือ plan
- ทดสอบบน server: office สร้าง user ระดับ office ได้, ตั้งบทบาท plan ถูกปฏิเสธ, แก้ไข/เปิดข้อมูล office ในหน่วยงานตนเองได้, ต่างหน่วยงานถูกบล็อก; ลบ test user แล้ว DB กลับปกติ (28/2/1/25/0)
- Deploy ขึ้น server แล้ว: `users.php`
